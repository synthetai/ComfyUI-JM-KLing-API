// JavaScript模块加载错误，改用正确的模块加载方式
// 不要使用import语句，而是通过app注册的方式
...
(() => {
    // 获取ComfyUI的app对象
    const app = Object.values(window).find(
        (m) => m && m.registerExtension && m.ComfyApp
    );
    
    if (!app) {
        console.error("[JM-KLingAI] 无法找到ComfyUI app对象，扩展加载失败");
        return;
    }
